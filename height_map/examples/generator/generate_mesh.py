...
if __name__ == '__main__':
    ...
    # Настройка сетки
    x = np.linspace(0, normalized_data.shape[1], normalized_data.shape[1])
    y = np.linspace(0, normalized_data.shape[0], normalized_data.shape[0])
    x, y = np.meshgrid(x, y)
    ...
    # Визуализация высотных данных
    ax.plot_surface(x, y, normalized_data * max_height, cmap='terrain', edgecolor='none')

    # Настройки осей и заголовка
    ax.set_title('3D Карта Высот')
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
